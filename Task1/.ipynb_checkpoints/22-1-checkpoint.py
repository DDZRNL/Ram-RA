####Only SBIR now
#### abstract can have "\n"
### The files can be dealed with: 22-1, 21-2,
    ...
    for i in range(len(divs1)):
        ...
        if (str is not None):
            if (str.find("Proposal Number") != -1):
                # Proposal number
                str = filterHTMLstr(divs1[i+1].strong.get_text())
                str = str.replace("22-1-", "22-1")
                dic["Proposal Number"]=str
            elif (str.find("Phase 1 Contract #") != -1):
                # Phase 1 Contract Number
                dic["Phase 1 Contract #"] = filterHTMLstr(divs1[i+1].strong.get_text())
            elif (str.find("Subtopic Title") != -1):
                # Subtopic Title
                dic["Subtopic Title"] = filterHTMLstr(divs1[i+1].strong.get_text())
            elif (str.find("Proposal Title") != -1):
                # Proposal Title
                dic["Proposal Title"] = filterHTMLstr(divs1[i+1].strong.get_text())
            elif (str.find("Firm") != -1):
                # Firm
...
def to_CSV(totaldata):
    pd.DataFrame(totaldata).to_csv('./SBIRResult/22_SBIR_phase1.csv',index=False)
...
if __name__ == '__main__':
    ...
    # SingleHTMLProcess(path)

    Directory_path = "../Datasets/22/sbir/phase1"
