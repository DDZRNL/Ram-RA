...
def SingleHTMLProcess(path):
    ...
    for i in range(len(heads)):
        if(heads[i] is not None):
            str=heads[i].get_text().strip()
            #if(str.find("SMALL BUSINESS CONCERN")!=-1 or str.find("PRINCIPAL INVESTIGATOR/PROJECT MANAGER") != -1 ):
            Head.append(str)
    ...
    if("SMALL BUSINESS CONCERN" in Head):
        ...
        dic["Principal Investigator_E-mail"] = filterHTMLstr(details[1])
        dic["Principal Investigator_Address"] = filterHTMLstr(details[2].rstrip() + ", " + details[3])
        dic["Principal Investigator_Phone"] = filterHTMLstr(details[4])
    ...
    if("SMALL BUSINESS CONCERN" in Head):
        ...
        str = str.replace("POTENTIAL NASA COMMERCIAL APPLICATIONS (LIMIT 100 WORDS)", '')
        dic["Potential NASA applications"] = filterHTMLstr(str)
        str = text[4].replace(text[5],'')
        str = str.replace("POTENTIAL NON-NASA COMMERCIAL APPLICATIONS (LIMIT 100 WORDS)", '')
...
def to_CSV(totaldata):
    pd.DataFrame(totaldata).to_csv('./SBIRResult/03_SBIR_phase2.csv',index=False, encoding="utf-8")
...
if __name__ == '__main__':
    Directory_path = "../Datasets/03/sbir/phase2"
    files_position = ReadFiles(Directory_path)
    totaldata=MultipleFileProcess(files_position)
    to_CSV(totaldata)
    #SingleHTMLProcess("Datasets/03/sbir/phase2/SBIR-03-2-A1.01-9927.html")
