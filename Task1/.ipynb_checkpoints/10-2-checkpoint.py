...
def SingleHTMLProcess(path):
    ...
    ###### TRL begin and end
    TRL_Begin = (details[9]).replace('\n', '').split()
    TRL_End = (details[10]).replace('\n', '').split()
...
def to_CSV(totaldata):
    pd.DataFrame(totaldata).to_csv('./SBIRResult/10_SBIR_phase2.csv',index=False)
...
if __name__ == '__main__':
    Directory_path = "../Datasets/2010/sbir/phase2"
