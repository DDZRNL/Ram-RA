    ...
    for i in range(len(tds)):
        ...
        if (tds[i] is not None):
            ...
            if (str.find("PROPOSAL NUMBER") != -1):
                ...
                index = str.find('-')
                ProNumber = str[:index + 1] + '1 ' + str[index + 1:]
...
def to_CSV(totaldata):
    pd.DataFrame(totaldata).to_csv('./SBIRResult/03_SBIR_phase1.csv',index=False, encoding="utf-8")
...
if __name__ == '__main__':
    Directory_path = "../Datasets/03/sbir/phase1"
