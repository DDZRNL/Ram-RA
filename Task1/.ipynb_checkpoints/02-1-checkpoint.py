...
def SingleHTMLProcess(path):
    ...
    str=str.replace("POTENTIAL COMMERCIAL APPLICATIONS (LIMIT 150 WORDS)",'')
    dic["Potential non-NASA applications"] = filterHTMLstr(str)
    str=text[4].replace(text[5],'')
    str = str.replace("POTENTIAL NASA APPLICATIONS (LIMIT 150 WORDS)",'')
...
def to_CSV(totaldata):
    pd.DataFrame(totaldata).to_csv('./SBIRResult/02_SBIR_phase1.csv',index=False, encoding="utf-8")
...
if __name__ == '__main__':
    Directory_path = "../Datasets/02/sbir/phase1"
