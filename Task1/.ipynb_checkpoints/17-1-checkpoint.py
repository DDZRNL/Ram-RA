...
def SingleHTMLProcess(path):
    ...
    for i in range(len(tds)):
        ...
        if (tds[i] is not None):
            ...
            if (str.find("PROPOSAL NUMBER") != -1):
                ...
                index = str.find('171')
                ProNumber = str[:index + 2] + '-' + str[index + 2:]
                dic["Proposal Number"]=ProNumber
    ...
    str=text[8].replace("TECHNOLOGY TAXONOMY MAPPING (NASA's technology taxonomy has been developed by the SBIR-STTR program to disseminate awareness of proposed and awarded R/R&D in the agency. It is a listing of over 100 technologies, sorted into broad categories, of interest to NASA.)","")
    list=str.split("\n")
    list1=[]
...
def to_CSV(totaldata):
    pd.DataFrame(totaldata).to_csv('./SBIRResult/17_SBIR_phase1.csv',index=False)
...
if __name__ == '__main__':
    Directory_path = "../Datasets/17/sbir/phase1"
