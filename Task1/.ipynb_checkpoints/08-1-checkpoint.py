...
def SingleHTMLProcess(path):
    ...
    #     print(i,details[i])
    if (details[0].find("(FAR 3.104)") != -1):
        dic["Small Business Concern_Firm"] = filterHTMLstr(details[1])
        dic["Small Business Concern_Address"] = filterHTMLstr(details[2].rstrip() + ", " + details[3])
        dic["Small Business Concern_Phone"] = filterHTMLstr(details[4])
        dic["Principal Investigator_Name"] = filterHTMLstr(details[5])
        dic["Principal Investigator_E-mail"] = filterHTMLstr(details[6])
        dic["Principal Investigator_Address"] = filterHTMLstr(details[7].rstrip() + ", " + details[8])
        dic["Principal Investigator_Phone"] = filterHTMLstr(details[9])
    else:
    ...
    if (text[0].find("SOURCE SELECTION INFORMATION - LIMITED USE AND DISTRIBUTION") != -1):
        ###### TRL end
        ...
        str = text[12].replace(
            "TECHNOLOGY TAXONOMY MAPPING",
            "")
        str=str.replace("NASA's technology taxonomy has been developed by the SBIR-STTR program to disseminate awareness of proposed and awarded R/R&D in the agency. It is a listing of over 100 technologies, sorted into broad categories, of interest to NASA.","")
...
def to_CSV(totaldata):
    pd.DataFrame(totaldata).to_csv('./SBIRResult/08_SBIR_phase1.csv', index=False)
...
if __name__ == '__main__':
    Directory_path = "../Datasets/08/sbir/phase1"
