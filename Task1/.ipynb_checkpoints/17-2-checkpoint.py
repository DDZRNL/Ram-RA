...
def SingleHTMLProcess(path):
    ...
    for i in range(len(paras)):
        if (paras[i] is not None):
            ...
            #print(i, " ", len(str)," ",str)
            text.append(str)
    dic["Technical Abstract"] = filterHTMLstr(text[6])
...
def to_CSV(totaldata):
    pd.DataFrame(totaldata).to_csv('./SBIRResult/17_SBIR_phase2.csv',index=False)
...
if __name__ == '__main__':
    Directory_path = "../Datasets/17/sbir/phase2"
