...
def SingleHTMLProcess(path):
    ...
    for i in range(len(tds)):
        ...
        if (tds[i] is not None):
            ...
            if (str.find("PROPOSAL NUMBER") != -1):
                ...
                index = str.find('05')
                ProNumber = str[:index + 2] + '-1' + str[index + 2:]
    ...
    str=str.replace("POTENTIAL NASA COMMERCIAL APPLICATIONS (LIMIT 150 WORDS)",'')
    dic["Potential NASA applications"] = filterHTMLstr(str)
    str=text[4].replace(text[5],'')
    str = str.replace("POTENTIAL NON-NASA COMMERCIAL APPLICATIONS (LIMIT 150 WORDS)",'')
    ...
    for i in range(len(list)):
        if (len(list[i]) > 0 and list[i].find("Form Generated on") == -1 and list[i].find("Form Printed on") == -1):
...
def to_CSV(totaldata):
    pd.DataFrame(totaldata).to_csv('./SBIRResult/05_SBIR_phase1.csv',index=False, encoding="utf-8")
...
if __name__ == '__main__':
    Directory_path = "../Datasets/05/sbir/phase1"
