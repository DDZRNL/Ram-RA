...
def SingleHTMLProcess(path):
    ...
    htmlfile = open(path, 'r', encoding='unicode_escape')
    ...
    for i in range(len(list)):
        if (len(list[i]) > 0 and list[i].find("Form Printed on") == -1):
...
def to_CSV(totaldata):
    pd.DataFrame(totaldata).to_csv('./SBIRResult/06_SBIR_phase1.csv',index=False, encoding="utf-8")
...
if __name__ == '__main__':
    Directory_path = "../Datasets/06/sbir/phase1"
