...
def SingleHTMLProcess(path):
    dic = {"Proposal Number": "", "Phase 1 Contract #": "", "Subtopic Title": "", "Proposal Title": "",
           ...
           "Potential non-NASA applications": "","Technology Taxonomy Mapping":""}
    ...
    for i in range(len(tds)):
        if (tds[i] is not None):
            ...
            if (str.find("PROPOSAL NUMBER") != -1):
                # Proposal number
                dic["Proposal Number"] = filterHTMLstr(tds[i].next_element.next_element.get_text())
            ...
            elif (str.find("PROJECT TITLE") != -1):
                # Proposal Title
                dic["Proposal Title"] = filterHTMLstr(tds[i].next_element.next_element.get_text())
    ...
    #             if (len(str) > 0):
    #                 details.append(str)
    details=paras[3].get_text().split('\n')
    details = [x for x in details if x != '']

    #print(dic["Proposal Number"])
    # for i in range(len(details)):
    #     print(i, len(details[i]),details[i])
    dic["Principal Investigator_Name"] = filterHTMLstr(details[1])
    dic["Principal Investigator_Address"] = filterHTMLstr(details[3].rstrip() + ", " + details[4])
    dic["Small Business Concern_Firm"] = filterHTMLstr(details[6])
    dic["Small Business Concern_Address"] = filterHTMLstr(details[7].rstrip() + ", " + details[8])
