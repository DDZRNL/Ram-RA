...
pageList = [i for i in range(601, 610)]
# pageList = [1, 2]
...
for i in pageList:
    ...
    for j in range(len(tr)):
        ...
        if counter % 50 == 0:
            page += 1
            pd.DataFrame(totalData).to_csv('task8-AdditionalSBC-Info' + str(page) + '.csv', index=False, encoding="utf-8")
            print(str(counter) + " finished" + ", page: " + str(page))
page += 1
pd.DataFrame(totalData).to_csv('task8-AdditionalSBC-Info' + str(page) + '.csv', index=False, encoding="utf-8")
print(str(counter) + " finished" + ", page: " + str(page))
