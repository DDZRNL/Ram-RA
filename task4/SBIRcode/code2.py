####Only SBIR now
#### abstract can have "\n"
### The files can be dealed with: 16,15,14
...
def SingleHTMLProcess(path):
    ...
    # HighTitles=bs.find_all("h2")   # Higher Topics
    # for i in range(len(HighTitles)):
    #     if(HighTitles[i] is not None):
    #         str=filterHTMLstr(HighTitles[i].get_text())
    #         if (str.find("TA") != -1):
    #             HigherTopics.append(str)
    #
    # for i in range(len(HigherTopics)):
    #     print(i, HigherTopics[i])

    Titles=bs.find_all("h4",{'class':"topicTitle"})  #  {'class': 'right-md-24 24'}
...
def to_CSV(totaldata):
    pd.DataFrame(totaldata).to_csv('2014.csv',index=False)

if __name__ == '__main__':
    # path="Datasets/22/sbir/phase1/SBIR-22-1-A1.02-1831.html"
    path="Datasets/2014.html"
